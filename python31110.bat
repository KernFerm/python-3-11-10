@echo off
echo This script will install Python 3.11.10 on your system.
...
bitsadmin /transfer "PythonDownloadJob" /download /priority normal https://www.python.org/ftp/python/3.11.10/python-3.11.10-amd64.exe "%cd%\python-3.11.10.exe"

:: Check if the download was successful
if not exist "%cd%\python-3.11.10.exe" (
...
"%cd%\python-3.11.10.exe" /quiet InstallAllUsers=0 PrependPath=1 Include_test=0

:: Check the result of the installation
if %ERRORLEVEL% equ 0 (
    echo Python 3.11.10 has been installed successfully.
) else (
